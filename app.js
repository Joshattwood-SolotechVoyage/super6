...
let session={role:null,userId:null};let activeLeague=state.leagues[0]?.id;let timer=null;let playerDraft=null;let accountManagerLoad=0;
let liveStandings=[];
...
function predictionLocked(){const c=cutoff();return !liveRoundExists||!c||Number.isNaN(c.getTime())||Date.now()>=c.getTime()}
...
function leagueWeeklyOutcome(leagueId){if(!state.round.completed)return null;const rows=eligiblePlayers(leagueId).map(p=>{const e=entryFor(p.id),s=scoreEntry(e);const diff=state.round.officialMinute==null?null:Math.abs(e.minute-state.round.officialMinute);return{p,e,...s,diff}});if(!rows.length)return{rows:[],first:[],second:[],spoons:[]};rows.sort((a,b)=>b.points-a.points||(a.diff??999)-(b.diff??999)||a.p.name.localeCompare(b.p.name));const topPts=rows[0].points;const topGroup=rows.filter(r=>r.points===topPts);let first=[];if(state.round.officialMinute==null){first=topGroup}else{const best=Math.min(...topGroup.map(r=>r.diff));first=topGroup.filter(r=>r.diff===best)}let second=[];if(first.length===1){const remaining=rows.filter(r=>r.p.id!==first[0].p.id);if(remaining.length){const secPts=remaining[0].points;const secGroup=remaining.filter(r=>r.points===secPts);if(state.round.officialMinute==null)second=secGroup;else{const best=Math.min(...secGroup.map(r=>r.diff));second=secGroup.filter(r=>r.diff===best)}}}
 const minPts=Math.min(...rows.map(r=>r.points));const spoons=rows.filter(r=>r.points===minPts);return{rows,first,second,spoons}}
...
async function login(name,pin){
 ...
 await Promise.all([refreshLiveStandings(), refreshLiveRound()]);
...
function renderPlayer(){const p=currentUser(),e=entryFor(p.id),live=currentLiveStanding();const rank=live?.position??seasonSortedLocal(p.leagueId).find(x=>x.id===p.id)?.pos??'-';$('#roundHeroName').textContent=liveRoundExists?state.round.name:'No round published';$('#playerLeagueName').textContent=live?.league_name||leagueName(p.leagueId);const pay=$('#paymentChip'),ent=$('#entryChip');if(!liveRoundExists){pay.textContent='Waiting';pay.className='chip';ent.textContent='No round';ent.className='chip';}else if(e.paid){pay.textContent='Paid ✓';pay.className='chip good'}else if(e.submitted&&predictionLocked()&&!paymentLocked()){pay.textContent='Payment pending';pay.className='chip warn'}else if(e.submitted&&paymentLocked()){pay.textContent='Entry not counted';pay.className='chip bad'}else{pay.textContent='Not paid';pay.className='chip bad'}
 ...
 $('#playerStats').innerHTML=[['Position',rank],['Points',live?.points??p.points],['Wins',live?.wins??p.wins],['Exact',live?.exact_scores??p.exact],['Results',live?.correct_results??p.correct]].map(x=>`<div class="stat"><strong>${x[1]}</strong><span>${x[0]}</span></div>`).join('');
...
function weeklyRankingHTML(out,meId=null){const ranked=weeklyPositions(out.rows);return `<div class="round-ranking">${ranked.map(r=>`<div class="round-rank-row ${r.p.id===meId?'me':''}"><div class="round-pos">${r.weekPos}</div><div class="round-person"><b>${r.p.name}<span class="award-mini">${weeklyAwardMark(out,r)}</span></b><small>${r.exact} exact · ${r.correct} correct result${state.round.officialMinute==null?'':` · Goal ${r.e.minute}'`}</small></div><div class="round-points">${r.points}<small style="display:block;font:11px Georgia;color:var(--muted);letter-spacing:.5px">PTS</small></div></div>`).join('')}</div>`}
function renderPersonalResult(){const sec=$('#personalResultSection'),p=currentUser(),out=leagueWeeklyOutcome(p.leagueId),e=entryFor(p.id);if(!state.round.completed){sec.classList.add('hidden');return}sec.classList.remove('hidden');if(!out||!out.rows.length){sec.innerHTML=`<div class="kicker">${state.round.name}</div><h3 style="margin:4px 0">Weekly league results</h3><div class="notice">No paid entries counted in ${leagueName(p.leagueId)} for this round.</div>`;return}const row=out.rows.find(r=>r.p.id===p.id);const ranked=weeklyPositions(out.rows),mine=ranked.find(r=>r.p.id===p.id);const personal=row?`<div class="stats" style="margin-bottom:14px"><div class="stat"><strong>${row.points}</strong><span>Your points</span></div><div class="stat"><strong>${mine.weekPos}</strong><span>Your finish</span></div><div class="stat"><strong>${row.exact}</strong><span>Exact</span></div><div class="stat"><strong>${row.correct}</strong><span>Results</span></div></div>`:`<div class="notice" style="margin-bottom:12px">Your entry did not count this week, but you can still see how your league finished.</div>`;sec.innerHTML=`<div class="weekly-league-title"><div><div class="kicker">${state.round.name}</div><h3>How everyone did</h3><div class="muted">${leagueName(p.leagueId)} · paid entries only</div></div><div class="muted">First goal ${state.round.officialMinute}'</div></div>${personal}${weeklyRankingHTML(out,p.id)}`;}
function renderLeagueTabs(){const holder=$('#leagueTabs');holder.innerHTML=state.leagues.map(l=>`<button type="button" data-league-tab="${l.id}" class="${activeLeague===l.id?'active':''}">${l.name}</button>`).join('');holder.querySelectorAll('button').forEach(b=>b.onclick=()=>{activeLeague=b.dataset.leagueTab;renderLeagueTabs()});const body=$('#leagueTableBody');const rows=liveRowsForLeague(activeLeague);if(rows.length){body.innerHTML=rows.map(r=>`<tr class="${r.player_id===session.authUserId?'me':''}"><td>${r.position}</td><td>${r.username}</td><td>${r.points}</td><td>${r.wins}</td><td>${r.exact_scores}</td><td>${r.correct_results}</td><td>${r.weeks_played}</td><td>${r.wooden_spoons}</td></tr>`).join('');return}const me=currentUser()?.id;body.innerHTML=seasonSortedLocal(activeLeague).map(p=>`<tr class="${p.id===me?'me':''}"><td>${p.pos}</td><td>${p.name}</td><td>${p.points}</td><td>${p.wins}</td><td>${p.exact}</td><td>${p.correct}</td><td>${p.played}</td><td>${p.spoons}</td></tr>`).join('')}
...
function renderAdmin(){renderAdminHero();renderFixtureEditor();renderAdminLeaguePanels();renderResultsEditor();renderWeeklyResults();renderLeagueManagement();renderRealAccountManager()}
function renderAdminHero(){if(session.role!=='admin')return;const rs=$('#roundState');if(!liveRoundExists){$('#mSubmitted').textContent='0';$('#mPaid').textContent='0';$('#mUnpaid').textContent='0';$('#mMissing').textContent=liveStandings.length||state.players.length;$('#adminCutoff').textContent='No round published · create the next fixture sheet below';rs.textContent='New round';rs.className='chip';return;}const players=adminRoundOverview.players||[];$('#mSubmitted').textContent=players.filter(p=>p.submitted).length;$('#mPaid').textContent=players.filter(p=>p.paid).length;$('#mUnpaid').textContent=players.filter(p=>p.submitted&&!p.paid).length;$('#mMissing').textContent=players.filter(p=>!p.submitted).length;$('#adminCutoff').textContent=`${state.round.name} · cutoff ${fmtDate(cutoff())} · payment grace to ${fmtDate(graceEnd())}`;if(predictionLocked()){rs.textContent=paymentLocked()?'Locked':'Predictions locked · payment grace';rs.className='chip warn'}else{rs.textContent='Open';rs.className='chip good'}}
...
function renderFixtureEditor(){$('#adminRoundName').value=state.round.name||'';$('#adminCutoffInput').value=state.round.cutoff?localDT(state.round.cutoff):'';const holder=$('#fixtureEditor');const fixtures=state.round.fixtures?.length===6?state.round.fixtures:cleanRoundDraft().fixtures;holder.innerHTML=fixtures.map((f,i)=>`<div class="admin-fixture"><input data-fi="${i}" data-k="home" value="${escapeAttr(f.home||'')}" placeholder="Home team" aria-label="Home team"><input data-fi="${i}" data-k="away" value="${escapeAttr(f.away||'')}" placeholder="Away team" aria-label="Away team"></div>`).join('');const locked=liveRoundExists&&predictionLocked();holder.querySelectorAll('input').forEach(inp=>inp.disabled=locked);$('#adminRoundName').disabled=locked;$('#adminCutoffInput').disabled=locked;$('#saveRoundBtn').disabled=locked;$('#saveRoundBtn').style.opacity=locked?.45:1}
...
function renderAdminLeaguePanels(){
 ...
 if(!liveRoundExists){wrap.innerHTML='<div class="notice">Publish the next round above to start tracking submissions and payment.</div>';return;}
 ...
 wrap.innerHTML=leagues.map(l=>{
  ...
  return `<div class="league-panel"><h4>${escapeHtml(l.name)}</h4><div style="padding:9px 10px;font-size:12px"><b>${sub}</b> submitted · <b>${paid}</b> paid · <b>${unpaid}</b> unpaid · <b>${miss}</b> not entered</div>${ps.map(p=>`<div class="player-row"><button class="secondary small name" data-view="${p.id}" type="button" style="text-align:left"><b>${escapeHtml(p.username)}</b><small>${p.submitted?'Submitted':'Not entered'}${p.paid?' · Paid':p.submitted?' · Unpaid':''}</small></button><label class="switch"><input type="checkbox" data-paid="${p.id}" ${p.paid?'checked':''} ${paymentLocked()?'disabled':''}> Paid</label><span class="chip ${p.submitted?'good':'bad'}">${p.submitted?'✓':'—'}</span></div>`).join('')}<div class="money">Pot currently: <strong>£${counted*state.round.fee}</strong></div></div>`;
 }).join('');
 wrap.querySelectorAll('[data-paid]').forEach(x=>x.onchange=async()=>{
  const desired=x.checked,playerId=x.dataset.paid;
  x.disabled=true;
  try{await window.Super6Backend.setAdminPayment(state.round.id,playerId,desired);await refreshAdminRoundOverview();renderAdminHero();renderAdminLeaguePanels()}
...
async function adminViewPlayer(playerId){
 ...
 if(!p)return;
 modal(`<div class="section-head"><div><div class="kicker">Admin entry view</div><h3>${escapeHtml(p.username)}</h3><div class="muted">${escapeHtml(p.league_name||'')} · ${p.paid?'Paid':'Not paid'}</div></div><button class="secondary small" id="closeAdminView">Close</button></div><div id="liveAdminEntry"><div class="muted">Loading live entry…</div></div>`);
 ...
 try{
  ...
  box.innerHTML=`${matches}<div class="review-row"><span>First-goal minute</span><b>${live.entry.first_goal_minute??'–'}</b></div><div class="notice" style="margin-top:12px">This is the live Supabase entry. Admin prediction editing will be connected separately before we enable result completion.</div>`;
...
function completeRound(){let ok=true;$('#resultsEditor').querySelectorAll('input[data-ri]').forEach(inp=>{const f=state.round.fixtures[+inp.dataset.ri];if(f.removed)return;if(inp.value==='')ok=false;else{f.result=f.result||[0,0];f.result[+inp.dataset.rs]=Math.max(0,Math.min(20,+inp.value))}});const min=$('#officialMinute').value;if(min==='')ok=false;if(!ok){$('#resultValidation').textContent='Enter both scores for every active fixture and the official first-goal minute.';return}state.round.officialMinute=Math.max(1,Math.min(90,+min));const was=state.round.completed;state.round.completed=true;state.round.completedAt=new Date().toISOString();if(was)state.round.audit.push({time:new Date().toISOString(),text:'Completed round corrected and recalculated by Admin'});save();renderAdmin();$('#resultValidation').textContent='Round calculated. Payment changes during the grace window will recalculate automatically.'}
function applyRoundToDisplayOnly(){save()}
function renderWeeklyResults(){const box=$('#weeklyResults'),money=$('#moneySummary');if(!liveRoundExists){box.innerHTML='<div class="notice">No round published yet.</div>';money.innerHTML='';$('#posterBtn').disabled=true;return;}if(!state.round.completed){box.innerHTML='<div class="notice">Complete the round to calculate league winners.</div>';money.innerHTML='';$('#posterBtn').disabled=true;return}$('#posterBtn').disabled=false;box.innerHTML=state.leagues.map(l=>{const o=leagueWeeklyOutcome(l.id);if(!o.rows.length)return`<div class="result-card"><h4>${l.name}</h4><div class="muted">No eligible paid entries.</div></div>`;const names=a=>a.map(r=>`${r.p.name} (${r.points})`).join(', ');return`<div class="result-card"><h4>${l.name}</h4><div class="podium"><div><span>🏆 Winner</span><b>${names(o.first)}</b></div>${o.first.length===1&&o.second.length?`<div><span>🥈 2nd</span><b>${names(o.second)}</b></div>`:''}<div><span>🥄 Spoon</span><b>${names(o.spoons)}</b></div></div>${weeklyRankingHTML(o)}</div>`}).join('');money.innerHTML='<div class="money"><b>Admin-only money view</b>'+state.leagues.map(l=>`<div>${l.name}: ${eligiblePlayers(l.id).length} paid × £${state.round.fee} = <strong>£${eligiblePlayers(l.id).length*state.round.fee}</strong></div>`).join('')+'</div>'}
...
});$('#logoutBtn').onclick=logout;$('#whoBtn').onclick=logout;$$('[data-player-tab]').forEach(b=>b.onclick=()=>{$$('[data-player-tab]').forEach(x=>x.classList.toggle('active',x===b));$('#playerHome').classList.toggle('hidden',b.dataset.playerTab!=='home');$('#playerTables').classList.toggle('hidden',b.dataset.playerTab!=='tables');$('#playerHistory').classList.toggle('hidden',b.dataset.playerTab!=='history');if(b.dataset.playerTab==='tables')renderLeagueTabs()});$('#saveRoundBtn').onclick=saveRound;$('#completeRoundBtn').onclick=completeRound;$('#reopenRoundBtn').onclick=()=>{$('#resultValidation').textContent='You can correct the results above, then press “Save corrections & recalculate”.'};$('#posterBtn').onclick=showPoster;$('#addLeagueBtn').onclick=addLeague;$('#movePlayerBtn').onclick=movePlayer;
})();
