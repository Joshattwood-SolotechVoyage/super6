...
async function refreshLatestLeagueWinners(){
 try{
  ...
  latestLeagueWinners=data?.winners||[];
  latestLeagueWinnerRound=data?.round||null;
 }catch(err){
  console.warn('Could not load latest league winners',err);
  latestLeagueWinners=[];latestLeagueWinnerRound=null;
 }
 return latestLeagueWinners;
}
function isLatestLeagueWinner(playerId,playerName){
 const id=String(playerId||'');const name=String(playerName||'').trim().toLowerCase();
 return latestLeagueWinners.some(w=>(id&&String(w.player_id||'')===id)||(name&&String(w.username||'').trim().toLowerCase()===name));
}
function leagueCrownHTML(playerId,playerName){
 return isLatestLeagueWinner(playerId,playerName)?`<span class="league-crown" title="Winner of the latest completed league round${latestLeagueWinnerRound?.name?`: ${escapeAttr(latestLeagueWinnerRound.name)}`:''}" aria-label="Latest league round winner">👑</span>`:'';
}
function playerNameWithCrown(playerId,playerName){return `${escapeHtml(playerName)}${leagueCrownHTML(playerId,playerName)}`}
...
function weeklyRankingHTML(out,meId=null){
 ...
 return `<div class="round-ranking">${ranked.map(r=>{const tie=state.round.officialMinute==null?'':(r.e?.minute!=null?` · Goal ${r.e.minute}'`:(r.diff!=null?` · Tie-break ±${r.diff}`:''));return `<div class="round-rank-row ${r.p.id===meId?'me':''}"><div class="round-pos">${r.weekPos}</div><div class="round-person"><b>${playerNameWithCrown(r.p.id,r.p.name)}<span class="award-mini">${weeklyAwardMark(out,r)}</span></b><small>${r.exact} correct score${r.exact===1?'':'s'} · ${r.correct} correct result${r.correct===1?'':'s'}${tie}</small></div><div class="round-points">${r.points}<small style="display:block;font:11px Georgia;color:var(--muted);letter-spacing:.5px">PTS</small></div></div>`}).join('')}</div>`;
}
...
async function adminViewPlayer(playerId){
 ...
 modal(`<div class="section-head"><div><div class="kicker">Admin entry view</div><h3>${playerNameWithCrown(p.id,p.username)}</h3><div class="muted">${escapeHtml(p.league_name||'')} · ${p.paid?'Paid':'Not paid'}${headline}</div></div><button class="secondary small" id="closeAdminView">Close</button></div><div id="liveAdminEntry"><div class="muted">Loading live entry…</div></div>`);
...
function renderWeeklyResults(){
 ...
 const awardNames=a=>a.map(r=>`<span class="weekly-award-name">${playerNameWithCrown(r.p.id,r.p.name)} <b>${r.points} pts</b></span>`).join('');
