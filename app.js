...
function renderAdminTables(){
 ...
 body.innerHTML=rows.map(r=>`<tr><td>${r.position}</td><td>${escapeHtml(r.username)}</td><td>${r.points}</td><td>${r.wins}</td><td>${r.exact_scores}</td><td>${r.correct_results}</td><td>${r.weeks_played}</td><td>${r.wooden_spoons}</td></tr>`).join('');
}
