...
let session={role:null,userId:null};let activeLeague=state.leagues[0]?.id;let activeAdminTab='overview';let timer=null;let playerDraft=null;let accountManagerLoad=0;
let liveStandings=[];
...
function renderAdmin(){renderAdminHero();renderFixtureEditor();renderAdminLeaguePanels();renderResultsEditor();renderWeeklyResults();renderLeagueManagement();renderRealAccountManager();setAdminTab(activeAdminTab)}
...
function renderAdminLeaguePanels(){
 ...
 wrap.innerHTML=leagues.map(l=>{
  ...
  return `<div class="league-panel"><h4>${escapeHtml(l.name)}</h4><div class="status-summary"><span class="status-pill submitted"><b>${sub}</b> Submitted</span><span class="status-pill paid"><b>${paid}</b> Paid</span><span class="status-pill unpaid"><b>${unpaid}</b> Unpaid</span><span class="status-pill missing"><b>${miss}</b> Not entered</span></div>${ps.map(p=>`<div class="player-row"><button class="secondary small name" data-view="${p.id}" type="button" style="text-align:left"><b>${escapeHtml(p.username)}</b><span class="player-statuses"><span class="status-pill mini ${p.submitted?'submitted':'missing'}">${p.submitted?'Submitted':'Not entered'}</span>${p.paid?'<span class="status-pill mini paid">Paid</span>':p.submitted?'<span class="status-pill mini unpaid">Unpaid</span>':''}</span></button><label class="switch"><input type="checkbox" data-paid="${p.id}" ${p.paid?'checked':''} ${paymentLocked()?'disabled':''}> Paid</label></div>`).join('')}<div class="money">Pot currently: <strong>£${counted*state.round.fee}</strong></div></div>`;
 }).join('');
