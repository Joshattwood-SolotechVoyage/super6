...
function liveWeeklyOutcome(leagueId){
 const rows=liveWeeklyResults.filter(r=>r.league_id===leagueId).map(r=>{
...
function renderAdminLeaguePanels(){
 ...
 wrap.querySelectorAll('[data-paid]').forEach(x=>x.onchange=async()=>{
  ...
  x.disabled=true;
  try{await window.Super6Backend.setAdminPayment(state.round.id,playerId,desired);await refreshAdminRoundOverview();if(state.round.completed){await Promise.all([refreshLiveWeeklyResults(),refreshLiveStandings()])}renderAdminHero();renderAdminLeaguePanels();if(state.round.completed)renderWeeklyResults()}
...
async function completeRound(){
 ...
 try{
  ...
  await Promise.all([refreshLiveStandings(),refreshAdminRoundOverview(),refreshLiveWeeklyResults()]);
  renderAdmin();
...
function renderWeeklyResults(){
 ...
 box.innerHTML=state.leagues.map(l=>{const o=liveWeeklyOutcome(l.id);if(!o.rows.length)return`<div class="result-card"><h4>${l.name}</h4><div class="muted">No eligible paid entries.</div></div>`;const names=a=>a.map(r=>`${r.p.name} (${r.points})`).join(', ');return`<div class="result-card"><h4>${l.name}</h4><div class="podium"><div><span>🏆 Winner</span><b>${names(o.first)}</b></div>${o.first.length===1&&o.second.length?`<div><span>🥈 2nd</span><b>${names(o.second)}</b></div>`:''}<div><span>🥄 Spoon</span><b>${names(o.spoons)}</b></div></div>${weeklyRankingHTML(o)}</div>`}).join('');
 const players=adminRoundOverview.players||[];
 money.innerHTML='<div class="money"><b>Admin-only money view</b>'+state.leagues.map(l=>{const leagueId=(adminRoundOverview.leagues||[]).find(x=>x.name===l.name)?.id;const count=players.filter(p=>p.league_id===leagueId&&p.submitted&&p.paid).length;return`<div>${l.name}: ${count} paid entries × £${state.round.fee} = <strong>£${count*state.round.fee}</strong></div>`}).join('')+'</div>';
}
